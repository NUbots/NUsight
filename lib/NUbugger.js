...
NUbugger.prototype.onMessage = function (robotIP, message) {
	...
	if(true) {
		...
		// If we don't have a write stream make one
		if(this.ofstream[robotIP] === undefined) {
			try {
				fs.mkdirSync("logs");
			}
			catch(e) {}
			...
			this.ofstream[robotIP] = fs.createWriteStream("logs/" + robotIP + "/" + Date.now() + ".nbs");
		}

		var stream = this.ofstream[robotIP];

		var length = new Buffer(4);
		length.writeUInt32LE(message.length, 0);

		// Write the NBS format
		stream.write(length);
